...
export default function NewsRoomContentClient({
  ...
  useEffect(() => {
    if (!newsRoomData || newsRoomData.photoNewsData.length === 0) return;
    const photoInterval = setInterval(() => {
      setCurrentPhotoIndex((prevIndex) =>
        prevIndex === newsRoomData.photoNewsData.length - 1 ? 0 : prevIndex + 1
      );
    }, 4000);
    return () => clearInterval(photoInterval);
  }, [newsRoomData.photoNewsData.length]);

  useEffect(() => {
    if (!newsRoomData || newsRoomData.shortsData.length === 0) return;
    const shortsInterval = setInterval(() => {
      setCurrentShortsIndex((prevIndex) =>
        prevIndex === newsRoomData.shortsData.length - 1 ? 0 : prevIndex + 1
      );
    }, 5000);
    return () => clearInterval(shortsInterval);
  }, [newsRoomData.shortsData.length]);

  const handleSlideControl = (
    type: "photo" | "shorts",
    direction: "prev" | "next" | number
  ) => {
    if (!newsRoomData) return;
    if (type === "photo") {
      if (newsRoomData.photoNewsData.length === 0) return;
      if (direction === "prev")
        setCurrentPhotoIndex((prev) =>
          prev === 0 ? newsRoomData.photoNewsData.length - 1 : prev - 1
        );
      else if (direction === "next")
        setCurrentPhotoIndex((prev) =>
          prev === newsRoomData.photoNewsData.length - 1 ? 0 : prev + 1
        );
      else if (typeof direction === "number") setCurrentPhotoIndex(direction);
    } else {
      if (newsRoomData.shortsData.length === 0) return;
      if (direction === "prev")
        setCurrentShortsIndex((prev) =>
          prev === 0 ? newsRoomData.shortsData.length - 1 : prev - 1
        );
      else if (direction === "next")
        setCurrentShortsIndex((prev) =>
          prev === newsRoomData.shortsData.length - 1 ? 0 : prev + 1
        );
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-3 space-y-8">
        {/* 사진뉴스 섹션 */}
        {newsRoomData.photoNewsData.length > 0 && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">사진뉴스</h2>
              <div className="flex items-center space-x-2">
                <div className="flex space-x-1">
                  {newsRoomData.photoNewsData.map((_, index) => (
                    <button
                      key={`photo-dot-${index}`}
            ...
            <div className="relative overflow-hidden">
              ...
              >
                {newsRoomData.photoNewsData.map((item) => (
                  <Link
                    href={item.link}
                    key={item.id}
                    target="_blank"
        ...
        {/* 쇼츠 섹션 */}
        {newsRoomData.shortsData.length > 0 && (
          <div>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">쇼츠</h2>
              <div className="flex items-center space-x-2">
                <div className="flex space-x-1">
                  {newsRoomData.shortsData.map((_, index) => (
                    <button
            ...
            <div className="relative overflow-hidden">
              ...
              >
                {newsRoomData.shortsData.map((item) => (
                  <Link
                    href={item.link}
                    key={item.id}
                    target="_blank"
      ...
      <div className="lg:col-span-6">
        ...
        <div className="space-y-6">
          {newsRoomData.mainNewsData.map((news, index) => (
            <div
              key={news.id}
              className={`${
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-3">
              {newsRoomData.briefingData.map((item) => (
          ...
          <div>
            ...
            <div className="space-y-3">
              {newsRoomData.pressReleaseData.map((item) => (
      ...
      <div className="lg:col-span-3">
        ...
        <div className="space-y-6">
          {newsRoomData.factCheckData.map((item) => (
